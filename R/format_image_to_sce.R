...
#' Nucleus Compactness, Nucleus Axis Ratio, Cell Axis Ratio for INFORM format; 
#' Cell Size, Nucleus Size for HALO format) are stored under colData
#' 
#' @export
...
format_image_to_sce_jojo2 <- function(format = "INFORM", image, markers, dye_columns_interest = NULL, intensity_columns_interest) {
  ...
  if (format == "HALO"){
    ...
    image <- image[,c("Object.Id", "XMin", "XMax", "YMin", "YMax", "Cell.Area", "Nucleus.Area")]
    ...
    for (marker in markers) {
      ...
      if (length(rows_true_exp) != 0) {
        expression_status_cols[rows_true_exp,]$Phenotype <- 
          paste(expression_status_cols[rows_true_exp,]$Phenotype, phenotype, sep="")
      }
    }
    ...
    image <- image[,c("Cell.ID", "Phenotype", "Cell.X.Position", "Cell.Y.Position", "Cell.Area", "Nucleus.Area")]
  ...
  if (format == "INFORM"){
    ...
    coldata_cell_size <- formatted_data[,"Cell.Area"]
    coldata_nucleus_size <- formatted_data[,"Nucleus.Area"]
  }
